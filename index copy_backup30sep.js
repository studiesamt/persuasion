...
var tweet_text="";
var max_questions=0;
var foot_note="answer cannot be empty";
...
// find mission_id
 ...
 if ( mission_state == "chk") {
     ...
     if(ans_y == "" && ans_n == ""){
        foot_note="answer cannot be empty";
...
var getAllRecords_B = (pool, quer, callback) => {
  ...
  const pool = new sql1.ConnectionPool(config, err => {
    ...
    var query1 = "SELECT * FROM [dbo].[Practice_data] WHERE Task_type= "+task_type_val;
    console.log(query);
    //Call function twice.
    getAllRecords_B(pool, query, recs => {
      tweet_text=recs.recordset[0].Tweet_text;
      tweet_ans=recs.recordset[0].Answer;
      correct_text=recs.recordset[0].Correct_text;
      Wrong_text=recs.recordset[0].Wrong_text;

    });

    getAllRecords_B(pool, query1, recs => {
        max_questions=recs.recordsets[0].length;
      });
