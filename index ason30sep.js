...
app.use( function(req, res, next)  {
  var reqQueryObject = req.query // returns object with all parameters
   ...
   var mission_id=req.query.missionId
   var blockID='1'
...
if ( mission_state === "chk") {
    ...
    if(ans_y === "" && ans_n === ""){
       foot_note="Please choose an option, the answer cannot be empty. Does the tweet text answer to the definition?";
       console.log(foot_note);
    } 

    if(ans_y === "Yes" && ans_n === "No"){
        foot_note="Please don not choose both the options, the answer cannot be both. Does the tweet text answer to the definition?";
        console.log(foot_note);
     }
...
console.log("missin above undefined mission now "+mission_id);
...
if( mission_state === "bkd") {
    mission_id--;
    console.log("missin inside bkd now "+mission_id);
  ...
  var task_type_val=2
   ...
   //find tweet text
    ...
    dbConn.connect().then(function () {
        ...
        if( mission_state === "fwd") {
            ++mission_id;
            console.log("missin inside fwd now "+mission_id);
            
        } 
        
        request.query("SELECT * FROM [dbo].[Practice_data] WHERE Task_type="+task_type_val+"").then(function (resp) {
           // console.log("SELECT Tweet_text FROM [dbo].[Practice_data] WHERE Mission_ID= "+mission_id+" AND Task_type="+task_type_val+"");
           console.log(mission_id); 
           console.log("missin inside select query now "+mission_id);
           console.log(resp.recordset[mission_id].Tweet_text);
            tweet_text=resp.recordset[mission_id].Tweet_text;
        dbConn.close();
